...
First, we need a way to describe the state of the bridge at a given time. We can use a **struct** that includes fields for the two lights at the bridge, the sensors `W_A` and `W_B` described in specification **(2)**, and a `cross_counter` to keep track of how many cars have crossed at a certain light as described in specification **(7)**.

include_example(exercises/bridge/state.h)

Here, `LightA` and `LightB` will be set equal to 1 if they are red and 2 if they are green.
...
Now, try adding your own specifications to the following functions. Make sure that you specify a valid state as a pre and post condition for every function. If you get stuck, the solution is in the solutions folder.
