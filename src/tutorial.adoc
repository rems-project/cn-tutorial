...
//// 
    So lemmas are actually closer to escape hatches in the type
    system. If you delete the first 4 lines od the ensures, then you have
    a function that consumes those resources (e.g. by freeing them). If
    delete the NewQ == Q, then the SMT solver has no way of knowing that
    the value of the returned resource is in anyway related to the
    incoming one.

    Lemmas are just "trusted" C functions which can also take in ghost
    values. We could tidy this up a bit more once we have the "inv"
    syntax. But from the perspective of CN checking a function which calls
    a lemma, it's pretty much the same as a calling a C function -
    whatever the function body is, it's simply assumed to match the
