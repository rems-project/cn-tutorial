...
Suppose we are writing a program that simulates a traffic light
on a one way bridge and we want to verify that our traffic light always behaves
"safely". We can do this by implementing the following specifications in CN:

    1. At each end of the bridge (end A and end B), there is a
      traffic light that can be either red or green. The two lights
      can never be green at the same time.

    2. At each end of the bridge, there is also a sensor that can
      detect the number of cars currently waiting to cross. These
      can be modeled with counters W_A and W_B for the two ends.

    3. At any time, a car traveling from A to B is either waiting at
      A or has passed across the bridge. During a given “clock tick,"
      the following actions happen atomically:  The
      lights change (if necessary) and one car from one end may cross, decrementing
    ...
    4. Once a car is waiting at one end to use the bridge, it continues
      waiting until it crosses.
// TODO: BCP: Where is this modeled?  (More generally, can you point out the place in the formal spec where each clause of the informal one is expressed?)

    5. Both lights start out red, with no cars at either end.

    6. If there is at least one car waiting at A and no cars at B, the
      light at A turns green (and vice versa for end B).

    7. If both lights are red and cars arrive simultaneously at both
      ...
      clock ticks later.
// TODO: BCP: I don't see how the first sentence here is reflected in the model.

To encode all this in CN, we first need a way to describe the state of
the bridge at a given time. We can use a `+struct+` that includes
fields for the two lights at the bridge, the sensors `W_A` and `W_B`
described in specification clause (2), and a `cross_counter` to keep
track of how many cars have crossed at a certain light as described in
clause (7).
...
TODO: BCP:
  ...
  One thing that makes this whole example awkward is that we don't
  really have an established convention in CN for using constants.  I
  suggest we use this opportunity to create one.

  E.g., what do you think about this?

     #define              RED        1
       /*@ function (i32) RED ()   { 1i32 } @*/
     #define              GREEN      2
       /*@ function (i32) GREEN () { 2i32 } @*/
...
The convention is that `LightA` and `LightB` will be set equal to 1 if they are red and 2 if they are green.
...
include_example(exercises/bridge/valid_state.h)

\\ TODO: BCP: I think the last line says that both lights are allowed to be green if no cars are waiting, which contradicts the informal spec.

Let's walk through the specifications in `valid_state`:

  * The first two lines ensure that both traffic lights in our model behave as intended: they can only be 1 or 2, because a traffic light can only be red or green.
  ...
  * The fifth line says that the cross counter must be between 0 and 5. This will help with specification (7) that no car will wait longer than five clock ticks. However, when we implement our `tick` function, we will need to ensure that we only increment the cross counter if there is a car waiting at the other side!
// TODO: BCP: I find this encoding of the "at most five" constraint a bit hard to understand.  Could there be another way to express it?  (I feel like, in general, the model is missing the concept of "time".)
...
Next, we need a way to represent if cars are approaching the light at either side. We can create another `struct` to represent this. `A` will be 1 if a car is approaching side A, and 0 otherwise. The same can be said about light B.

\\ TODO: BCP: Could there be cars approaching both sides?  More than one on each side?
...
include_example(exercises/bridge/funcs1.h)
...
include_example(exercises/bridge/funcs2.c)
