...
/*@ requires valid_state(s);
             0i32 <= s.Runway_Time;
             s.Runway_Time <= 4i32; // TODO: might be 5?
             s.ModeA == ACTIVE() || s.ModeD == ACTIVE();
    ensures  valid_state(return);
...
// This function represents what happens every minute at the airport. One `tick` corresponds to one minute.
...
/* --END-- */
{
    ...
    // Plane chosen to arrive
      ...
      s = arrive(s);
      s = increment_Runway_Time(s); //todo
      return s;
    } 
