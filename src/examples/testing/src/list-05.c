...
// This fails because the spec says that insertion discards duplicates,
// but the implementation does not.
// Note that at present we sample types uniformly, so we have to use
// a relatively small type, such as `unsigned int` to catch the error,
// and even then we don't catch the error on each run.
